...
def decision_tree_classifier(train_x, train_y, num_classes=10):
	from sklearn import tree
	model = tree.DecisionTreeClassifier()
	model.fit(train_x, train_y)
	return model
	
def svm_classifier(train_x, train_y, num_classes=10):
	from sklearn.svm import SVC
	model = SVC(kernel='rbf', gamma=0.01, C=1) # 选择kernel之后，rbf的准确率比其他的高，参数还没研究过
	model.fit(train_x, train_y)
	return model
	
def cnn_classifier(train_x, train_y, num_classes=10):
...
def dnn_classifier(train_x, train_y, num_classes=10):
	batch_size = 100
	nb_classes = 10
	nb_epoch = 20
